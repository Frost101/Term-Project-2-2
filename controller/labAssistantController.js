...
async function getTests(req,res){
    const sql=`
    SELECT APPTID, TO_CHAR(APPT_DATE,'YYYY-MM-DD') APPT_DATE, APPT_TIME, LABNAME, TEST_NAME, PID
    FROM
    (SELECT *
    FROM APPT_TESTS APT_T
    JOIN APPOINTMENTS APT USING (APPTID)
    WHERE APT_T.DONE = 'N') T1
...
async function getSingleTest(req,res){
    const sql=`
    SELECT APPTID, TO_CHAR(APPT_DATE,'YYYY-MM-DD') APPT_DATE, APPT_TIME, LABNAME, TEST_NAME, PID, FIRST_NAME || ' ' || LAST_NAME PATIENT_NAME, GET_AGE(DOB) AGE, BLOOD_GROUP 
    FROM
    APPT_TESTS
    JOIN TESTS USING (TEST_NAME)
