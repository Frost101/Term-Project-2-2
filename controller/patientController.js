...
async function getUpcomingAppointments(req,res){
  const sql=`
    SELECT DISTINCT APPTID,APPT_DATE, APPT_TIME, FIRST_NAME ||' '||LAST_NAME DOCTOR_NAME, SPECIALITY, GET_DOC_DEGREES(DEID) DEGREES, HOSPITAL_NAME, BRANCH
    FROM
    (SELECT *
    FROM APPOINTMENTS
    WHERE PID = :PID
    ...
    ORDER BY APPT_DATE ASC, APPTID ASC) T1
