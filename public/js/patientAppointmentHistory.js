...
async function search(){
    let info = {
      ...
      let result = await fetch("http://localhost:4200/patient/getAppointmentHistory", {
          method: "POST",
...
function test(result) {
    ...
    for (i = 0; i < result.length; i++) {
      ...
      <div class="row g-0">
        ...
        <div class="card-body">
          ...
          <button type="button" class="btn btn-dark" onclick="clicked(this)">Open Prescription</button>
        </div>
        </div>
      </div>
      </div>`;
  ...
  function clicked(ref) {
    ...
    window.location.replace("http://localhost:4200/patient/prescription")
  }